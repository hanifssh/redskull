...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Bonk someone! .bonk @user or reply with .bonk',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to bonk!\n.bonk @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't bonk yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `bonk_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `bonk_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} bonked @${mentioned.split('@')[0]}`,
