...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Punch someone! .punch @user or reply with .punch',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to punch!\n.punch @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't punch yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `punch_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `punch_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} punched @${mentioned.split('@')[0]}`,
