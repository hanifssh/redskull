...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Clap for someone! .clap @user or reply with .clap',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.clap @user' }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/clap', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `clap_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `clap_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} clapped for @${mentioned.split('@')[0]}`,
                                   mentions: [senderJid, mentioned]
            }, { quoted: msg });

        } catch (err) {
            console.error('[clap] Error:', err.message);
