...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Shake someone! .shake @user or reply with .shake',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.shake @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't shake yourself!" }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/shake', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `shake_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `shake_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} shook @${mentioned.split('@')[0]}`,
                                   mentions: [senderJid, mentioned]
            }, { quoted: msg });

        } catch (err) {
            console.error('[shake] Error:', err.message);
