...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Cry with someone! .cry @user or reply with .cry',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to cry!\n.cry @user' }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `cry_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `cry_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} cried with @${mentioned.split('@')[0]}`,
