...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Dance with someone! .dance @user or reply with .dance',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to dance!\n.dance @user' }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `dance_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `dance_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} danced with @${mentioned.split('@')[0]}`,
