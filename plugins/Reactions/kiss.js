...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Kiss someone! .kiss @user or reply with .kiss',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to kiss!\n.kiss @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't kiss yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `kiss_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `kiss_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} kissed @${mentioned.split('@')[0]}`,
