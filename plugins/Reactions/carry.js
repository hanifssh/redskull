...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Carry someone! .carry @user or reply with .carry',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.carry @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't carry yourself!" }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/carry', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `carry_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `carry_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} carried @${mentioned.split('@')[0]}`,
                                   mentions: [senderJid, mentioned]
            }, { quoted: msg });

        } catch (err) {
            console.error('[carry] Error:', err.message);
