...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Highfive someone! .highfive @user or reply with .highfive',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to highfive!\n.highfive @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't highfive yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `highfive_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `highfive_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} highfived @${mentioned.split('@')[0]}`,
