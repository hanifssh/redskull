...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Wink at someone! .wink @user or reply with .wink',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to wink!\n.wink @user' }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/wink', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `wink_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `wink_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} winked at @${mentioned.split('@')[0]}`,
