...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Poke someone! .poke @user or reply with .poke',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to poke!\n.poke @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't poke yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `poke_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `poke_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} poked @${mentioned.split('@')[0]}`,
