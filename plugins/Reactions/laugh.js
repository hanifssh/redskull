...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Laugh with someone! .laugh @user or reply with .laugh',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to laugh!\n.laugh @user' }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `laugh_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `laugh_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} laughed with @${mentioned.split('@')[0]}`,
                                   mentions: [senderJid, mentioned]
            }, { quoted: msg });

        } catch (err) {
            console.error('[laugh] Error:', err.message);
