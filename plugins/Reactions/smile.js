...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Smile at someone! .smile @user or reply with .smile',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to smile!\n.smile @user' }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/smile', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `smile_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `smile_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} smiled at @${mentioned.split('@')[0]}`,
