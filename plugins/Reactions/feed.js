...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Feed someone! .feed @user or reply with .feed',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.feed @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't feed yourself!" }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/feed', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `feed_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `feed_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} fed @${mentioned.split('@')[0]}`,
