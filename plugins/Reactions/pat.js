...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Pat someone! .pat @user or reply with .pat',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to pat!\n.pat @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't pat yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `pat_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `pat_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} patted @${mentioned.split('@')[0]}`,
