...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Thumbs up someone! .thumbsup @user or reply with .thumbsup',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.thumbsup @user' }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/thumbsup', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `thumbsup_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `thumbsup_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} gave thumbs up to @${mentioned.split('@')[0]}`,
                                   mentions: [senderJid, mentioned]
            }, { quoted: msg });

        } catch (err) {
            console.error('[thumbsup] Error:', err.message);
