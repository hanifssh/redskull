...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Sleep with someone! .sleep @user or reply with .sleep',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.sleep @user' }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/sleep', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `sleep_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `sleep_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} slept with @${mentioned.split('@')[0]}`,
