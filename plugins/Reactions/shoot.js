...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Shoot someone! .shoot @user or reply with .shoot',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.shoot @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't shoot yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `shoot_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `shoot_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} shot @${mentioned.split('@')[0]}`,
