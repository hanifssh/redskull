...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Slap someone! .slap @user or reply with .slap',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to slap!\n.slap @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't slap yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `slap_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `slap_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} slapped @${mentioned.split('@')[0]}`,
