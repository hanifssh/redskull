...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Hug someone! .hug @user or reply with .hug',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to hug!\n.hug @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't hug yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `hug_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `hug_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} hugged @${mentioned.split('@')[0]}`,
