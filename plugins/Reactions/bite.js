...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Bite someone! .bite @user or reply with .bite',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to bite!\n.bite @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't bite yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `bite_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `bite_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} bit @${mentioned.split('@')[0]}`,
