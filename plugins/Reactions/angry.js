...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Be angry at someone! .angry @user or reply with .angry',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.angry @user' }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/angry', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `angry_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `angry_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} is angry at @${mentioned.split('@')[0]}`,
                                   mentions: [senderJid, mentioned]
            }, { quoted: msg });

        } catch (err) {
            console.error('[angry] Error:', err.message);
