...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Cuddle someone! .cuddle @user or reply with .cuddle',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to cuddle!\n.cuddle @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't cuddle yourself!" }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `cuddle_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `cuddle_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} cuddled @${mentioned.split('@')[0]}`,
