...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Blush at someone! .blush @user or reply with .blush',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to blush!\n.blush @user' }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `blush_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `blush_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} blushed at @${mentioned.split('@')[0]}`,
