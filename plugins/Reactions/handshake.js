...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Handshake with someone! .handshake @user or reply with .handshake',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone!\n.handshake @user' }, { quoted: msg });
        }

        if (mentioned === senderJid) {
            return sock.sendMessage(from, { text: "❌ You can't handshake yourself!" }, { quoted: msg });
        }

        let tmpGif = '', tmpMp4 = '';
        try {
            const { data } = await axios.get('https://nekos.best/api/v2/handshake', {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `handshake_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `handshake_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} shook hands with @${mentioned.split('@')[0]}`,
