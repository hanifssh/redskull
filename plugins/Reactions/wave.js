...
module.exports = {
    ...
    aliases: [],
    category: 'Reactions',
    desc: 'Wave at someone! .wave @user or reply with .wave',
    ...
    execute: async (sock, from, msg, args) => {
        ...
        if (!mentioned) {
            return sock.sendMessage(from, { text: '❌ Mention or reply to someone to wave!\n.wave @user' }, { quoted: msg });
        }
        ...
        try {
            ...
            const id = Date.now();
            tmpGif = path.join(TEMP_DIR, `wave_${id}.gif`);
            tmpMp4 = path.join(TEMP_DIR, `wave_${id}.mp4`);
            ...
            await sock.sendMessage(from, {
                video: fs.readFileSync(tmpMp4),
                                   gifPlayback: true,
                                   caption: `@${senderJid.split('@')[0]} waved at @${mentioned.split('@')[0]}`,
