...
module.exports = {
    ...
    aliases: [],
    category: 'Group',
    desc: 'Kick a member from the group (admin/sudo/owner only)',
    execute: async (sock, from, msg, args, perms) => {
        ...
        if (!targetJid)
            return sock.sendMessage(from, { text: `❌ Please mention or reply to the user you want to kick.\nUsage: \`${prefix}kick @user\` or reply to their message.` });

        if (isSudoOrOwner(targetJid))
            return sock.sendMessage(from, { text: '❌ Cannot kick superusers or owner.' });

        try {
            await sock.groupParticipantsUpdate(from, [targetJid], 'remove');
            await sock.sendMessage(from, {
                text: `👢 @${targetJid.split('@')[0]} has been kicked.`,
                                   mentions: [targetJid]
            });
        } catch (err) {
            await sock.sendMessage(from, {
                text: `❌ Failed to kick @${targetJid.split('@')[0]}. Make sure I'm an admin.`,
                                   mentions: [targetJid]
            });
        }
    }
