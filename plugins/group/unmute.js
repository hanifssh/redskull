...
module.exports = {
    ...
    execute: async (sock, from, msg, args, perms) => {
        ...
        if (!allowed)
            return sock.sendMessage(from, { text: '❌ Only *Group Admins*, *Sudo*, or *Owner* can unmute the group.' });

        if (!meta.restrict)
            return sock.sendMessage(from, { text: '✅ Group is already unmuted.' });

        try {
            await sock.groupSettingUpdate(from, 'not_announcement');
            await sock.sendMessage(from, { text: '🔊 Group unmuted. All members can send messages again.' });
        } catch (err) {
            await sock.sendMessage(from, { text: '❌ Failed to unmute the group. Make sure I am an admin.' });
        }
    }
};
